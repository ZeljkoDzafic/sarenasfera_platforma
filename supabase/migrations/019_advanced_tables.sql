...
CREATE POLICY IF NOT EXISTS "parent_view_own_activity_media"
  ON public.home_activity_media FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "staff_admin_all_activity_media"
  ON public.home_activity_media FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_view_own_daily_logs"
  ON public.daily_logs FOR SELECT USING (
    child_id = ANY(public.my_children_ids())
  );

CREATE POLICY IF NOT EXISTS "staff_admin_all_daily_logs"
  ON public.daily_logs FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_view_own_meals"
  ON public.daily_log_meals FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.daily_logs dl WHERE dl.id = log_id AND dl.child_id = ANY(public.my_children_ids()))
  );

CREATE POLICY IF NOT EXISTS "staff_admin_all_meals"
  ON public.daily_log_meals FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_view_own_naps"
  ON public.daily_log_naps FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.daily_logs dl WHERE dl.id = log_id AND dl.child_id = ANY(public.my_children_ids()))
  );

CREATE POLICY IF NOT EXISTS "staff_admin_all_naps"
  ON public.daily_log_naps FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_view_own_checkins"
  ON public.check_ins FOR SELECT USING (
    child_id = ANY(public.my_children_ids())
  );

CREATE POLICY IF NOT EXISTS "staff_admin_all_checkins"
  ON public.check_ins FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_manage_own_pickups"
  ON public.authorized_pickups FOR ALL USING (
    child_id = ANY(public.my_children_ids())
  );

CREATE POLICY IF NOT EXISTS "staff_admin_view_pickups"
  ON public.authorized_pickups FOR SELECT USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_view_own_incidents"
  ON public.incident_reports FOR SELECT USING (
    child_id = ANY(public.my_children_ids())
  );

CREATE POLICY IF NOT EXISTS "staff_admin_all_incidents"
  ON public.incident_reports FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_manage_own_emergency_contacts"
  ON public.emergency_contacts FOR ALL USING (
    child_id = ANY(public.my_children_ids())
  );

CREATE POLICY IF NOT EXISTS "staff_admin_view_emergency_contacts"
  ON public.emergency_contacts FOR SELECT USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_view_own_stories"
  ON public.learning_stories FOR SELECT USING (
    child_id = ANY(public.my_children_ids()) AND is_shared = true
  );

CREATE POLICY IF NOT EXISTS "staff_admin_all_stories"
  ON public.learning_stories FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_view_own_story_media"
  ON public.learning_story_media FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "staff_admin_all_story_media"
  ON public.learning_story_media FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "parent_manage_portfolio_settings"
  ON public.child_portfolio_settings FOR ALL USING (
    child_id = ANY(public.my_children_ids())
  );

CREATE POLICY IF NOT EXISTS "staff_admin_view_portfolio_settings"
  ON public.child_portfolio_settings FOR SELECT USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "admin_all_inquiries"
  ON public.enrollment_inquiries FOR ALL USING (public.is_admin());

CREATE POLICY IF NOT EXISTS "staff_view_inquiries"
  ON public.enrollment_inquiries FOR SELECT USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "staff_admin_all_waitlist"
  ON public.waitlist FOR ALL USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "staff_view_own_schedule"
  ON public.staff_schedules FOR SELECT USING (staff_id = auth.uid());

CREATE POLICY IF NOT EXISTS "admin_all_schedules"
...
CREATE POLICY IF NOT EXISTS "staff_manage_own_time"
  ON public.staff_time_entries FOR ALL USING (staff_id = auth.uid());

CREATE POLICY IF NOT EXISTS "admin_all_time_entries"
  ON public.staff_time_entries FOR ALL USING (public.is_admin());
...
CREATE POLICY IF NOT EXISTS "staff_manage_own_goals"
  ON public.educator_goals FOR ALL USING (staff_id = auth.uid());

CREATE POLICY IF NOT EXISTS "admin_all_goals"
  ON public.educator_goals FOR ALL USING (public.is_admin());
...
CREATE POLICY IF NOT EXISTS "all_read_forum_categories"
  ON public.forum_categories FOR SELECT USING (true);

CREATE POLICY IF NOT EXISTS "admin_manage_forum_categories"
  ON public.forum_categories FOR ALL USING (public.is_admin());
...
CREATE POLICY IF NOT EXISTS "admin_all_activity_log"
  ON public.activity_log FOR ALL USING (public.is_admin());

CREATE POLICY IF NOT EXISTS "staff_view_activity_log"
  ON public.activity_log FOR SELECT USING (public.is_staff_or_admin());
...
CREATE POLICY IF NOT EXISTS "admin_all_ratios"
  ON public.room_ratios FOR ALL USING (public.is_admin());

CREATE POLICY IF NOT EXISTS "staff_view_ratios"
  ON public.room_ratios FOR SELECT USING (public.is_staff_or_admin());
