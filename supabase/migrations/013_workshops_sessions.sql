...
CREATE POLICY IF NOT EXISTS "workshops_auth_read"     ON public.workshops          FOR SELECT USING (auth.uid() IS NOT NULL AND is_published = true);
CREATE POLICY IF NOT EXISTS "workshops_admin_all"     ON public.workshops          FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
CREATE POLICY IF NOT EXISTS "materials_auth_read"     ON public.workshop_materials FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "materials_admin_all"     ON public.workshop_materials FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Sessions: parent reads sessions for their children's groups
CREATE POLICY IF NOT EXISTS "sessions_parent_read" ON public.sessions
  FOR SELECT USING (
    group_id IN (
...
CREATE POLICY IF NOT EXISTS "sessions_staff_all" ON public.sessions
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Attendance: parent sees own children; staff sees all
CREATE POLICY IF NOT EXISTS "attendance_parent_read" ON public.attendance
  FOR SELECT USING (
    child_id IN (SELECT child_id FROM public.parent_children WHERE parent_id = auth.uid())
    OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff')
  );
CREATE POLICY IF NOT EXISTS "attendance_staff_all" ON public.attendance
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Observations: parent sees visible observations of own children
CREATE POLICY IF NOT EXISTS "observations_parent_read" ON public.observations
  FOR SELECT USING (
    (child_id IN (SELECT child_id FROM public.parent_children WHERE parent_id = auth.uid()) AND is_visible_to_parent = true)
    OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff')
  );
CREATE POLICY IF NOT EXISTS "observations_staff_all" ON public.observations
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Observation media follows observation visibility
CREATE POLICY IF NOT EXISTS "obs_media_parent_read" ON public.observation_media
  FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "obs_media_staff_all" ON public.observation_media
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Assessments: parent reads; staff writes
CREATE POLICY IF NOT EXISTS "assessments_parent_read" ON public.assessments
  FOR SELECT USING (
    child_id IN (SELECT child_id FROM public.parent_children WHERE parent_id = auth.uid())
    OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff')
  );
CREATE POLICY IF NOT EXISTS "assessments_staff_all" ON public.assessments
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Reports: parent reads published reports for own children; staff manages
CREATE POLICY IF NOT EXISTS "reports_parent_read" ON public.quarterly_reports
  FOR SELECT USING (
    (child_id IN (SELECT child_id FROM public.parent_children WHERE parent_id = auth.uid()) AND status = 'published')
    OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff')
  );
CREATE POLICY IF NOT EXISTS "reports_staff_all" ON public.quarterly_reports
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Observation templates: authenticated read; admin/staff write
CREATE POLICY IF NOT EXISTS "obs_templates_read"  ON public.observation_templates FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "obs_templates_staff" ON public.observation_templates FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
