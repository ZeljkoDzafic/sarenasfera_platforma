...
CREATE POLICY IF NOT EXISTS "home_activities_parent" ON public.home_activities
  FOR SELECT USING (parent_id = auth.uid() OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
CREATE POLICY IF NOT EXISTS "home_activities_parent_write" ON public.home_activities
  FOR INSERT WITH CHECK (parent_id = auth.uid());
CREATE POLICY IF NOT EXISTS "home_activities_parent_update" ON public.home_activities
  FOR UPDATE USING (parent_id = auth.uid());
CREATE POLICY IF NOT EXISTS "home_activities_staff" ON public.home_activities
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Blog: public read published; admin manages
CREATE POLICY IF NOT EXISTS "blog_public_read"  ON public.blog_posts FOR SELECT USING (is_published = true);
CREATE POLICY IF NOT EXISTS "blog_admin_all"    ON public.blog_posts FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Resources: public read published/free; paid requires auth+tier
CREATE POLICY IF NOT EXISTS "resources_public_read" ON public.resources FOR SELECT USING (is_published = true AND requires_auth = false);
CREATE POLICY IF NOT EXISTS "resources_auth_read"   ON public.resources FOR SELECT USING (is_published = true AND auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "resources_admin_all"   ON public.resources FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Weekly challenges: public read
CREATE POLICY IF NOT EXISTS "challenges_public_read" ON public.weekly_challenges FOR SELECT USING (is_active = true);
CREATE POLICY IF NOT EXISTS "challenges_admin"       ON public.weekly_challenges FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Challenge submissions: parent writes own; admin manages
CREATE POLICY IF NOT EXISTS "submissions_parent_write" ON public.challenge_submissions FOR INSERT WITH CHECK (family_id = auth.uid());
CREATE POLICY IF NOT EXISTS "submissions_approved_read" ON public.challenge_submissions FOR SELECT USING (is_approved = true OR family_id = auth.uid());
CREATE POLICY IF NOT EXISTS "submissions_admin"        ON public.challenge_submissions FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Feedback: parent writes own; staff reads
CREATE POLICY IF NOT EXISTS "feedback_parent_write" ON public.workshop_feedback FOR INSERT WITH CHECK (parent_id = auth.uid());
CREATE POLICY IF NOT EXISTS "feedback_parent_read"  ON public.workshop_feedback FOR SELECT USING (parent_id = auth.uid() OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
