...
CREATE POLICY IF NOT EXISTS "skill_areas_public_read" ON public.skill_areas FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "skills_public_read"      ON public.skills      FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "curriculum_auth_read"    ON public.curriculum_lessons FOR SELECT USING (auth.uid() IS NOT NULL);

-- Admin/staff manage curriculum
CREATE POLICY IF NOT EXISTS "skill_areas_admin" ON public.skill_areas      FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
CREATE POLICY IF NOT EXISTS "skills_admin"      ON public.skills            FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
CREATE POLICY IF NOT EXISTS "curriculum_admin"  ON public.curriculum_lessons FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Child milestones: parent sees own children; staff sees all
CREATE POLICY IF NOT EXISTS "milestones_parent_read" ON public.child_milestones
  FOR SELECT USING (
...
CREATE POLICY IF NOT EXISTS "milestones_staff_write" ON public.child_milestones
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

CREATE POLICY IF NOT EXISTS "lesson_records_parent" ON public.child_lesson_records
  FOR SELECT USING (
    child_id IN (SELECT child_id FROM public.parent_children WHERE parent_id = auth.uid())
    OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff')
  );

CREATE POLICY IF NOT EXISTS "lesson_records_staff" ON public.child_lesson_records
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
