...
CREATE POLICY IF NOT EXISTS "lead_activities_admin" ON public.lead_activities FOR ALL USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Referrals: user sees own; admin sees all
CREATE POLICY IF NOT EXISTS "referrals_own"   ON public.referrals FOR SELECT USING (referrer_id = auth.uid() OR auth.jwt()->'app_metadata'->>'role' = 'admin');
CREATE POLICY IF NOT EXISTS "referrals_insert" ON public.referrals FOR INSERT WITH CHECK (referrer_id = auth.uid());
CREATE POLICY IF NOT EXISTS "referrals_admin"  ON public.referrals FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Pioneer wall: public read
CREATE POLICY IF NOT EXISTS "pioneer_wall_public" ON public.pioneer_wall FOR SELECT USING (is_visible = true);
CREATE POLICY IF NOT EXISTS "pioneer_wall_admin"  ON public.pioneer_wall FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Forum: authenticated read published
CREATE POLICY IF NOT EXISTS "forum_topics_read"  ON public.forum_topics FOR SELECT USING (is_published = true AND auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "forum_topics_write" ON public.forum_topics FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "forum_topics_admin" ON public.forum_topics FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
CREATE POLICY IF NOT EXISTS "forum_posts_read"   ON public.forum_posts  FOR SELECT USING (auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "forum_posts_write"  ON public.forum_posts  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);
CREATE POLICY IF NOT EXISTS "forum_posts_own"    ON public.forum_posts  FOR UPDATE USING (author_id = auth.uid());
CREATE POLICY IF NOT EXISTS "forum_posts_admin"  ON public.forum_posts  FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
