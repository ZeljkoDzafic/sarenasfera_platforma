...
DROP POLICY IF EXISTS "milestones_public_read" ON public.developmental_milestones;
CREATE POLICY "milestones_public_read" ON public.developmental_milestones FOR SELECT
  USING (is_active = true);

-- Milestones: admin can manage
DROP POLICY IF EXISTS "milestones_admin_all" ON public.developmental_milestones;
CREATE POLICY "milestones_admin_all" ON public.developmental_milestones FOR ALL
  USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Child milestones: parents see own children
DROP POLICY IF EXISTS "child_milestones_parent_read" ON public.child_milestones;
CREATE POLICY "child_milestones_parent_read" ON public.child_milestones FOR SELECT
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "child_milestones_staff_read" ON public.child_milestones;
CREATE POLICY "child_milestones_staff_read" ON public.child_milestones FOR SELECT
  USING (
    auth.jwt()->'app_metadata'->>'role' IN ('staff', 'admin')
  );

-- Child milestones: staff can insert/update
DROP POLICY IF EXISTS "child_milestones_staff_write" ON public.child_milestones;
CREATE POLICY "child_milestones_staff_write" ON public.child_milestones FOR ALL
  USING (
    auth.jwt()->'app_metadata'->>'role' IN ('staff', 'admin')
  );
