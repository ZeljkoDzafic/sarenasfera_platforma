...
DROP POLICY IF EXISTS "parent_children_own" ON public.parent_children;
...
DROP POLICY IF EXISTS "parent_children_admin" ON public.parent_children;
CREATE POLICY "parent_children_admin" ON public.parent_children
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
