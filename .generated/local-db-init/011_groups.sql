...
DROP POLICY IF EXISTS "group_staff_own" ON public.group_staff;
CREATE POLICY "group_staff_own" ON public.group_staff
  FOR SELECT USING (
    staff_id = auth.uid()
    OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff')
  );

DROP POLICY IF EXISTS "group_staff_admin" ON public.group_staff;
CREATE POLICY "group_staff_admin" ON public.group_staff
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Child groups: parent sees their children's groups; staff sees all
DROP POLICY IF EXISTS "child_groups_parent" ON public.child_groups;
CREATE POLICY "child_groups_parent" ON public.child_groups
  FOR SELECT USING (
    child_id IN (SELECT child_id FROM public.parent_children WHERE parent_id = auth.uid())
    OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff')
  );

DROP POLICY IF EXISTS "child_groups_admin" ON public.child_groups;
CREATE POLICY "child_groups_admin" ON public.child_groups
  FOR ALL USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
