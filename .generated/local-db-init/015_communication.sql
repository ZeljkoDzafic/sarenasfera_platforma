...
DROP POLICY IF EXISTS "campaigns_admin" ON public.email_campaigns;
CREATE POLICY "campaigns_admin" ON public.email_campaigns FOR ALL USING (auth.jwt()->'app_metadata'->>'role' = 'admin');
DROP POLICY IF EXISTS "recipients_admin" ON public.email_recipients;
CREATE POLICY "recipients_admin" ON public.email_recipients FOR ALL USING (auth.jwt()->'app_metadata'->>'role' = 'admin');
