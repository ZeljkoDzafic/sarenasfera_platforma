...
CREATE POLICY "forum_categories_public_read" ON public.forum_categories FOR SELECT
  USING (is_active = true);

DROP POLICY IF EXISTS "forum_categories_staff_write" ON public.forum_categories;
CREATE POLICY "forum_categories_staff_write" ON public.forum_categories FOR ALL
...
CREATE POLICY "forum_topics_authenticated_read" ON public.forum_topics FOR SELECT
  USING (
    (status = 'published' AND auth.uid() IS NOT NULL)
    OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff')
  );

DROP POLICY IF EXISTS "forum_topics_users_insert" ON public.forum_topics;
CREATE POLICY "forum_topics_users_insert" ON public.forum_topics FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "forum_topics_users_update" ON public.forum_topics;
CREATE POLICY "forum_topics_users_update" ON public.forum_topics FOR UPDATE
...
CREATE POLICY "forum_posts_authenticated_read" ON public.forum_posts FOR SELECT
  USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "forum_posts_users_insert" ON public.forum_posts;
CREATE POLICY "forum_posts_users_insert" ON public.forum_posts FOR INSERT
  WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "forum_posts_users_update" ON public.forum_posts;
CREATE POLICY "forum_posts_users_update" ON public.forum_posts FOR UPDATE
  USING (author_id = auth.uid() OR auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));
...
DROP POLICY IF EXISTS "forum_likes_authenticated" ON public.forum_likes;
CREATE POLICY "forum_likes_authenticated" ON public.forum_likes FOR ALL
  USING (auth.uid() IS NOT NULL)
  WITH CHECK (auth.uid() IS NOT NULL);

-- Forum bookmarks: users manage own
DROP POLICY IF EXISTS "forum_bookmarks_own" ON public.forum_bookmarks;
CREATE POLICY "forum_bookmarks_own" ON public.forum_bookmarks FOR ALL
...
DROP POLICY IF EXISTS "forum_reports_admin" ON public.forum_reports;
CREATE POLICY "forum_reports_admin" ON public.forum_reports FOR ALL
  USING (auth.jwt()->'app_metadata'->>'role' IN ('admin', 'staff'));

-- Forum reputation: public read
DROP POLICY IF EXISTS "forum_reputation_public" ON public.forum_reputation;
CREATE POLICY "forum_reputation_public" ON public.forum_reputation FOR SELECT
  USING (true);
...
DROP POLICY IF EXISTS "partner_referrals_partner_read" ON public.partner_referrals;
CREATE POLICY "partner_referrals_partner_read" ON public.partner_referrals FOR SELECT
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "partner_referrals_admin" ON public.partner_referrals;
CREATE POLICY "partner_referrals_admin" ON public.partner_referrals FOR ALL
  USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Partner payouts: partner sees own, admin sees all
DROP POLICY IF EXISTS "partner_payouts_partner_read" ON public.partner_payouts;
CREATE POLICY "partner_payouts_partner_read" ON public.partner_payouts FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM public.partners p
...
DROP POLICY IF EXISTS "partner_payouts_admin" ON public.partner_payouts;
CREATE POLICY "partner_payouts_admin" ON public.partner_payouts FOR ALL
  USING (auth.jwt()->'app_metadata'->>'role' = 'admin');
