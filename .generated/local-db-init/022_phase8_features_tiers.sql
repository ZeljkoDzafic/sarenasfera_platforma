...
CREATE POLICY "feature_flags_public_read" ON public.feature_flags FOR SELECT
  USING (true);
...
DROP POLICY IF EXISTS "feature_interests_insert" ON public.feature_interests;
CREATE POLICY "feature_interests_insert" ON public.feature_interests FOR INSERT
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "feature_interests_read_own" ON public.feature_interests;
CREATE POLICY "feature_interests_read_own" ON public.feature_interests FOR SELECT
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "feature_interests_admin_read" ON public.feature_interests;
CREATE POLICY "feature_interests_admin_read" ON public.feature_interests FOR SELECT
  USING (auth.jwt()->'app_metadata'->>'role' = 'admin');
...
DROP POLICY IF EXISTS "certificates_parents_read" ON public.certificates;
CREATE POLICY "certificates_parents_read" ON public.certificates FOR SELECT
  USING (
...
CREATE POLICY "certificates_staff_write" ON public.certificates FOR ALL
...
CREATE POLICY "badges_public_read" ON public.badges FOR SELECT
  USING (true);
...
CREATE POLICY "user_badges_own_read" ON public.user_badges FOR SELECT
  USING (user_id = auth.uid() OR auth.jwt()->'app_metadata'->>'role' = 'admin');
