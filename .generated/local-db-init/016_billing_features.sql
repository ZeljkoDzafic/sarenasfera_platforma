...
CREATE POLICY "plans_admin_all" ON public.subscription_plans FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Subscriptions: user sees own; admin sees all
DROP POLICY IF EXISTS "subs_own_read" ON public.subscriptions;
CREATE POLICY "subs_own_read" ON public.subscriptions FOR SELECT USING (user_id = auth.uid() OR auth.jwt()->'app_metadata'->>'role' = 'admin');
DROP POLICY IF EXISTS "subs_admin_all" ON public.subscriptions;
CREATE POLICY "subs_admin_all" ON public.subscriptions FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Payments: user sees own; admin manages
DROP POLICY IF EXISTS "payments_own" ON public.payments;
CREATE POLICY "payments_own" ON public.payments FOR SELECT USING (user_id = auth.uid() OR auth.jwt()->'app_metadata'->>'role' = 'admin');
DROP POLICY IF EXISTS "payments_admin" ON public.payments;
CREATE POLICY "payments_admin" ON public.payments FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');
...
CREATE POLICY "flags_public_read" ON public.feature_flags FOR SELECT USING (status IN ('active', 'coming_soon'));
DROP POLICY IF EXISTS "flags_admin_all" ON public.feature_flags;
CREATE POLICY "flags_admin_all" ON public.feature_flags FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Pioneer slots: public read; admin updates
DROP POLICY IF EXISTS "pioneer_public_read" ON public.pioneer_slots;
CREATE POLICY "pioneer_public_read" ON public.pioneer_slots FOR SELECT USING (true);
DROP POLICY IF EXISTS "pioneer_admin" ON public.pioneer_slots;
CREATE POLICY "pioneer_admin" ON public.pioneer_slots FOR ALL    USING (auth.jwt()->'app_metadata'->>'role' = 'admin');

-- Feature interests: users can insert their own; users see own; admin sees all
DROP POLICY IF EXISTS "interests_user_insert" ON public.feature_interests;
CREATE POLICY "interests_user_insert" ON public.feature_interests FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "interests_own_read" ON public.feature_interests;
CREATE POLICY "interests_own_read" ON public.feature_interests FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "interests_admin_read" ON public.feature_interests;
CREATE POLICY "interests_admin_read" ON public.feature_interests FOR SELECT USING (auth.jwt()->'app_metadata'->>'role' = 'admin');
